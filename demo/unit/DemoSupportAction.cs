...
public class DemoSupportAction : IUnitAction<CombatAction>
{
    ...
    public CombatAction Perform(UnitData unit, Vector2I target)
    {
    ...
    public void UpdateGrid(GridData grid, CombatAction results)
    {
        // Get the version of the action's actor on the input grid to avoid updating the wrong grid
        UnitData target = grid.Occupants[results.Target.Cell];
