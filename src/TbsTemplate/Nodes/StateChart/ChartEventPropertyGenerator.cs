...
/// <summary>Generator for editor properties that contain an event from a <see cref="Chart"/>.</summary>
/// <param name="properties">List of property data to use for generation.</param>
public class ChartEventPropertyGenerator(params ChartEventPropertyGenerator.ChartEventProperty[] properties)
{
    ...
    /// <summary>Create a new <see cref="Chart"/> event generator for an object.</summary>
    /// <param name="this">Object containing properties to generate.</param>
    /// <param name="properties">Names of properties to generate.</param>
    public ChartEventPropertyGenerator(object @this, params StringName[] properties)
        : this(properties.Select((p) => new ChartEventProperty(@this, p)).ToArray()) {}

    /// <param name="state">State chart defining events to store in the properties.</param>
    /// <returns>
    /// A property list containing all the state chart event properties that can be used in
    /// <see cref="GodotObject._GetPropertyList"/>.
    /// </returns>
    public Godot.Collections.Array<Godot.Collections.Dictionary> GetChartEventProperties(Chart state)
    {
        ...
        list.AddRange(properties.Select((p) => state.CreateEventProperty(p.Name).ToDictionary()));
    ...
    {
        foreach (ChartEventProperty p in properties)
        {
            if (p.Name == property)
            {
                value = p.Get();
