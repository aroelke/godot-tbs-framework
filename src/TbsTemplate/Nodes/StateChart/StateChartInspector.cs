...
public partial class StateChartInspector : MarginContainer
{
    ...
    public override void _Process(double delta)
    {
        ...
        if (StateChart.GetExpressionProperties().Any())
        {
            ImmutableList<StringName> properties = StateChart.GetExpressionProperties().ToImmutableList().Sort(static (a, b) => string.Compare(a, b));
            ...
            foreach (StringName property in properties)
            {
                ...
                line.SetText(0, $"{property} = {StateChart.GetExpressionProperty(property)}");
            }
        }
    }
}