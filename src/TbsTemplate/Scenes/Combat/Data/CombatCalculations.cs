...
public static class CombatCalculations
{
    ...
    public static int Damage(UnitState attacker, UnitState defender) => Math.Max(attacker.Stats.Attack - defender.Stats.Defense, 0);
    ...
    public static int HitChance(UnitState attacker, UnitState defender) => attacker.Stats.Accuracy - defender.Stats.Evasion;
    ...
    public static CombatAction CreateAttackAction(UnitState attacker, UnitState defender) => new(attacker, defender, CombatActionType.Attack, Damage(attacker, defender), rnd.Next(100) < HitChance(attacker, defender));
    ...
    public static CombatAction CreateSupportAction(UnitState supporter, UnitState recipient) => new(supporter, recipient, CombatActionType.Support, -supporter.Stats.Healing, true);
    ...
    /// </returns>
    public static (UnitState doubler, UnitState other)? FollowUp(UnitState a, UnitState b) => (a.Stats.Agility - b.Stats.Agility) switch
    {
        >0 => (a, b),
        <0 => (b, a),
        _  => null
    ...
    public static List<CombatAction> AttackResults(UnitState a, UnitState b)
    {
        ...
        if (FollowUp(a, b) is (UnitState doubler, UnitState doublee))
    ...
    public static int TotalDamage(UnitState target, IEnumerable<CombatAction> actions) => actions.Where((a) => a.Hit && a.Target == target).Select((a) => a.Damage).Sum();
    ...
    public static double TotalExpectedDamage(UnitState target, IEnumerable<CombatAction> actions) => actions.Where((a) => a.Target == target).Select((a) => a.Damage*HitChance(a.Actor, target)/100.0).Sum();
}